...
class ObtenerPublicacion {
    ...
    public function obtenerPublicacionesPorUsuario($idUsuarioPublicador) {
        $conexion = $this->conectar->getConexion();
        $sql = "SELECT * FROM vista_publicaciones_usuario WHERE usuario_publicador = ?";
        $stmt = $conexion->prepare($sql);
        $stmt->bind_param("i", $idUsuarioPublicador);
        $stmt->execute();
